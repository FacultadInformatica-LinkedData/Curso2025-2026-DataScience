...
Original file is located at
    https://colab.research.google.com/drive/11Qc4SktgQOgaEHgEBt7404BQQydL4pUq
"""
...
# TO DO
personas= Namespace("http://oeg.fi.upm.es/resource/person/")
oscar = personas.Oscar
g.add((personas.Oscar, RDF.type, people.FullProfessor))
g.add((personas.Oscar, RDFS.label, Literal("Oscar", datatype=XSD.string)))
g.add((personas.Oscar, people.hasColleague, personas.Asun))
g.add((personas.Oscar, people.hasName, Literal("Oscar García", datatype=XSD.string)))
asun = personas.Asun
g.add((personas.Asun, RDF.type, people.FullProfessor))
g.add((personas.Asun, RDFS.label, Literal("Asun", datatype=XSD.string)))
g.add((personas.Asun, people.hasColleague, oscar))
g.add((personas.Asun, people.hasHomePage, Literal("https://www.oeg-upm.net/", datatype=XSD.string)))
raul = personas.Raul
g.add((personas.Raul, RDFS.label, Literal("Raul", datatype=XSD.string)))
g.add((personas.Raul, RDF.type, people.InterimAssociateProfessor))
# Visualize the results
for s, p, o in g:
  print(s,p,o)
...
g.add((personas.Oscar, vcard.Given, Literal("Oscar", datatype=XSD.string)))
g.add((personas.Oscar, vcard.Family, Literal("García", datatype=XSD.string)))
g.add((personas.Oscar, foaf.email, Literal("oscar@upm.es", datatype=XSD.string)))
# Visualize the results
for s, p, o in g:
  print(s,p,o)
